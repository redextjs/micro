  ...
  if (hasRegisterApp) {
    registerApplication({
      ...
      app: async () => {
        ...
        let { entry, queryString, loadScriptPath, loadScriptId } = appConfig;
        ...
        if (loadScriptUrl) {
          if (queryString) {
            loadScriptUrl += `${loadScriptUrl.includes('?') ? '&' : '?'}${queryString}`
          }
          
          if (appVersion) {
            loadScriptUrl += `${loadScriptUrl.includes('?') ? '&' : '?'}v=${appVersion}`
          }
          ...
          lifeCycles = await loadScript({
            ...
            fetchOptions: {
              attrs: {
                id: loadScriptId || 'redext-micro-app-script'
              }
            }
          });
